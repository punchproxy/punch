...
export default function Sessions() {
  ...
  return <>
    ...
    <div className="toolbar"><input className="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter by host, relay, rule, process…" aria-label="Filter sessions"/><select value={filter} onChange={(event) => setFilter(event.target.value)} aria-label="Session status"><option value="all">All</option><option value="active">Active only</option><option value="closed">Closed only</option></select><div className="spacer"/><button className="btn btn-sm btn-danger-ghost" disabled={!activeCount} onClick={terminateAll}>Terminate all active</button></div>
    <Card><div className="table-wrap"><table className="data responsive-table sessions-table"><thead><tr><th>Destination</th><th>Proto</th><th>Relay</th><th>Rule</th><th>Process</th><th>↑ Up</th><th>↓ Down</th><th>Duration</th><th>Status</th><th>Actions</th></tr></thead><tbody>
      {rows.map((session) => <SessionRow key={session.id} session={session} terminate={terminate} showTrace={showTrace}/>)}
      {!rows.length && <tr className="empty-row"><td colSpan="10"><Empty>No sessions match.</Empty></td></tr>}
