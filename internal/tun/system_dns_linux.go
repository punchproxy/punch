...
func linuxCurrentResolvConfDNS() ([]systemDNSState, error) {
	...
	return []systemDNSState{{
		...
		Link:    linuxResolvConfLinkTarget(),
	}}, nil
}

// linuxResolvConfLinkTarget reports the symlink target of resolv.conf, or ""
// when it is a regular file. This fallback runs precisely when
// systemd-resolved is broken or stopped, which is also when resolv.conf is
// most likely a dangling link into /run/systemd/resolve.
func linuxResolvConfLinkTarget() string {
	info, err := os.Lstat(linuxResolvConfPath)
	if err != nil || info.Mode()&os.ModeSymlink == 0 {
		return ""
	}
	target, err := os.Readlink(linuxResolvConfPath)
	if err != nil {
		return ""
	}
	return target
...
func linuxApplyResolvConfDNSOverride(states []systemDNSState, serverIP string) error {
	content := "# Generated by Punch. Original resolv.conf will be restored when Punch stops.\n" +
		"nameserver " + serverIP + "\n"
	if err := linuxReplaceResolvConf(content, ""); err != nil {
...
func linuxRestoreResolvConfDNS(states []systemDNSState) error {
	...
	if err := linuxReplaceResolvConf(content, link); err != nil {
		return fmt.Errorf("restore %s: %w", linuxResolvConfPath, err)
	}
	return nil
}

// linuxReplaceResolvConf replaces resolv.conf itself rather than writing
// through it. os.WriteFile follows a symlink and would truncate whatever it
// points at — typically a systemd-owned file under /run. Staging next to the
// target and renaming over it also makes the swap atomic for readers, and
// works whether the existing entry is a file or a link.
//
// When link is non-empty the replacement is a symlink to that target;
// otherwise it is a regular file holding content.
func linuxReplaceResolvConf(content, link string) error {
	tmp := linuxResolvConfPath + ".punch-tmp"
	...
	} else if err := os.WriteFile(tmp, []byte(content), 0o644); err != nil {
		return err
	}
	if err := os.Rename(tmp, linuxResolvConfPath); err != nil {
		_ = os.Remove(tmp)
		return err
	}
	return nil
